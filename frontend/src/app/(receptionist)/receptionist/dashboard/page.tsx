...
export default function ReceptionistDashboard() {
  ...
  const loadData = useCallback(async () => {
    try {
      const [apptRes, queueRes] = await Promise.all([
        api.get("/appointment"),
        api.get("/queue/doctor/fac099b7-0edf-41a7-95ce-43549cb62451"), // TODO: dynamic in Phase 8
      ]);
  ...
  // SignalR — listen for queue updates
  useEffect(() => {
    ...
    connection.on("QueueUpdated", () => {
      // Reload queue when any update comes in
      api.get("/queue/doctor/fac099b7-0edf-41a7-95ce-43549cb62451")
    ...
    connection
      ...
      .then(async () => {
        ...
        await connection.invoke(
          "JoinDoctorQueue",
          "fac099b7-0edf-41a7-95ce-43549cb62451"
        );
      })
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 320px", gap: "24px" }}>
        ...
        <div>
          ...
          <div style={{ marginTop: "16px" }}>
            <AppointmentTable
              title={`Pending Check-in (${pending.length})`}
          ...
          {done.length > 0 && (
            <div style={{ marginTop: "16px" }}>
              <AppointmentTable
                title={`Completed / Cancelled (${done.length})`}
                appointments={done}
                checkingIn={checkingIn}
                onCheckIn={handleCheckIn}
                muted
                ...
                {queue.map((entry) => (
                  <div
                    ...
                    style={{
                      ...
                      backgroundColor:
                        entry.status === "in_progress" ? "#f0f8f4" : "#fff",
                    }}
                  >
                    <div
                      style={{
                    ...
                    >
                      <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                        <span
                          style={{
                            ...
                            borderRadius: "50%",
                            backgroundColor:
                              entry.status === "in_progress" ? "#1a5c9a" : "#e0e4e8",
                            color:
                              entry.status === "in_progress" ? "#fff" : "#333",
                            display: "flex",
                            alignItems: "center",
                      ...
                      <span
                        style={{
                          ...
                          fontWeight: 600,
                          color:
                            entry.status === "in_progress" ? "#1a7a4a" : "#666",
                        }}
                      >
...
function AppointmentTable({
  ...
  return (
    <div
      style={{
        backgroundColor: "#fff",
        border: `1px solid ${highlight ? "#1a5c9a44" : "#c8d0d8"}`,
        borderRadius: "4px",
      }}
    >
      <div
        style={{
          padding: "12px 16px",
          borderBottom: "1px solid #e0e4e8",
          backgroundColor: highlight ? "#f0f4f8" : "#f8f9fa",
        }}
      >
        <span style={{ fontSize: "13px", fontWeight: 600, color: "#333" }}>
          {title}
            ...
            {appointments.map((appt, i) => {
              ...
              return (
                ...
                >
                  ...
                  <td style={{ padding: "10px 16px", color: muted ? "#aaa" : "#444" }}>
                    {appt.doctorName}
                  </td>
                  ...
                  <td style={{ padding: "10px 16px" }}>
                    {appt.status === "scheduled" || appt.status === "confirmed" ? (
                      <button
                        ...
                        style={{
                          backgroundColor: isCheckingIn ? "#4a7aaa" : "#1a5c9a",
                          color: "#fff",
                          border: "none",
                          padding: "5px 12px",
                          borderRadius: "3px",
